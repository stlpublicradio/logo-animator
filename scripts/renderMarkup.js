import { animationsMarkup } from './markupSnippets'
...
const renderSVG = options => {
  const { animation, colors, form } = options
  const animationCSS = animationsMarkup[animation] || ''
  ...
  <svg class="logo" fill="${form}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 90 35.47">
    ...
    <g class="logo-bars" fill="${isGradient ? 'url(#gradient-fill)' : firstColor}">
...
const renderHTML = options => {
  const { animation, colors, form } = options
  return `<style></style>${animation} - ${colors} - ${form}`
}

const renderMarkup = options => {
  const { mode } = options
  if (mode === 'SVG') return renderSVG(options)
  if (mode === 'HTML') return renderHTML(options)
}
